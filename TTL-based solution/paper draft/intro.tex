...
\subsection{Proof of Deadlock-free}\label{subsec:proof}
...
\begin{enumerate}
	...
	\item \textbf{Finite packet scheduling time:} We assume that packet scheduling functions can decide the next packet to be processed within finite time. Formally, $f^{in}_{sche}$ and $f^{out}_{sche}$ should satisfy the following constraints at any network node:
		\begin{align} \label{eqn:schecon}
    	t_{cost}(f^{in}_{sche}(q_{in}^{i})) < \infty ,1\leq i \leq n\\
	    t_{cost}(f^{out}_{sche}(q_{out}^{i})) < \infty, 1\leq i \leq n
		\end{align}
   ...
   \begin{align} \label{eqn:legalstatecon}
    ...
    |q_{in}^{i,j}|\leq t_{PFC}, \quad 1\leq i \leq n, 1\leq j \leq k
   \end{align}
...
%      \textbf{}
    ...
    \textbf{Proof:} To prove our TTL-based solution is deadlock-free, we prove by contradiction that no legal buffer state $BS_N(t)$ is deadlocked buffer state under our TTL-based solution.
   ...
   According to Equations (6)-(9), any unscheduled packet remaining in any VIQ will be forwarded to some VIQ within some finite time. Assuming that there is no packet
   
   As we have $\forall t_1>t_0, BS_N(t_1)\equiv BS_N(t_0) \quad \text{and} \quad BS_N(t_0) \neq BS^0_N.$ All the packets remaining in $N$ must have been forwarded to their destined VEQs. Otherwise, $BS_N(t_0)$  buffer
