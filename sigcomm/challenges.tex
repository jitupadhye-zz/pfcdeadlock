...
Incremental deployment means a few practical constraints.

First, we do not want to force the network operator to change the topology or routing configuration.
Existing routing configuration has been designed in a way that best fit the needs. 
Changing it may cause loss of performance, {\em e.g.,} limit the number of multipaths, or increase
complexity and reduce reliability.

Unfortunately, the common network topology and routing, even a Clos network with BGP routing, has 
the risk of having cyclic buffer dependency (Section~\ref{sec:reroute}).

Second, the deadlock prevention solution must be decentralized. A centralized controller for deadlock
prevention requires significant changes in the network.

Finally, we do not change the packet header formats.

\subsection{Common IP Routing is Dynamic}\label{sec:reroute}

This is due to the fact that IP routing protocols 

In a Clos based network \cite{fat-tree,vl2}, if packets always follow the UP-DOWN routing, then deadlock cannot happen as CBD is not possible. In up-down routing, a packet first goes UP from the source server to one of the common ancestor switches of the source and destination servers, then it goes DOWN from the common ancestor to the destination server.
...
Packet reroutes may happen due to several reasons including link flapping and routing protocol flapping \cite{f10}. 
