...
Here we describe several possible ways of doing so: First, we can assign different PFC thresholds to the ports of a switch based on their positions. Ports connecting to the downstream get smaller threshold, whereas ports connecting to the upstream get larger threshold. %different tiers and make PFC happen less on higher tier. Also, higher tier switches may be allowed to ignore PFC or drop packets from lower tier upon extreme cases.
