...
However, this cyclic buffer dependency may not always turn into that state. The flow
...
In our simulations, we configure static routing on all switches and have flow 1 start at a host (not shown) 
attached to A, passes through B and C, and end at a host attached to D. Flow 2 starts at a host attached 
to C, passes through D and A, and ends at a host attached to B.  To evaluate whether there will be 
deadlock in the worst case, both flows are UDP flows with infinite traffic demand. Link capacity of 
...
Buffer dependency graph of case 1 is drawn in Figure~\ref{fig:case1}(b). In the figure, RX represents input 
queue (or port), while TX represents output queue (or port). Each directed line represents 
a buffer dependency from the source TX to the destination TX. For example, packets buffered in TX1 of 
A will be sent to either TX1 or TX2 of B, so in Figure~\ref{fig:case1}(b), two directed lines are drawn 
there between A and B. Similarly, we can draw the dependency lines between other switches. As we can 
see in Figure~\ref{fig:case1}(b), there is a cyclic buffer dependency among the four switches, i.e., 
dependencies from TX1 of A to TX1 of B, then to TX1 of C, then to TX1 of D, and finally back to TX1 of A.

In Figure~\ref{fig:case1}(c), we plot the PFC pause events at four links L1, L2, L3 and L4. Basically, 
...
To understand the pause pattern in Figure~\ref{fig:case1}(c), we sample the instant buffer occupancy of 
both flows at TX1 queues of A, B, C and D every 1us. In Figure~\ref{fig:case1}(d), we draw the instant 
buffer occupancy of flow 2 at TX1 of A. Buffer occupancy of flow 1 is not drawn in Figure~\ref{fig:case1}(d) 
...
As shown in Figure~\ref{fig:case1}(d), buffer occupancy of flow 2 at TX1 of A fluctuates between 10KB 
...
\textbf{Slightly different traffic matrix leads to deadlock:} as shown in Figure~\ref{fig:case2}(a), based on the previous case, we 
...
Compared with case 1, one additional dependency from TX1 of B to TX1 of C is added, but it
is outside the cyclic buffer dependency. The cyclic buffer dependency remains unchanged.
...
describe the PFC behaviors in this example. Looking at the packet traces, we only roughly know that 
